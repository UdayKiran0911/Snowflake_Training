-- Performance Aspects
    -- Dedicated Virtual warehouses: Seperated according to different workloads
        -- Identify & Clasify groupds of workloads/users
        -- Create desdicated warehouses for every class of workload and assign users
        -- CONSIDERATIONS:
            -- Not too many warehouses
            -- Refine Classifications
    -- Scaling UP: For known patterns of high work loads
        -- Query Complexity
        -- Increase Warehouse size
        -- CONSIDERATIONS:
            -- Must have Enterprise edition or better
            -- Min = 1, Max could be very high
    -- Scaling OUT: Dynamically for unknown patterns of work loads
        -- More Users/ concurrent queries
        -- Multi-clusters
    -- Maximize Cache Usage: Automatic caching can be maximized
        -- Automatical process to speed up the queries
        -- if the query is executed twice, the result is cached for re-use
        -- Results are cached for 24 hrs, or utill the underlying data has changed
...
// Re execute to see the query duration
